...
Tiefensuche`` Prozess alle möglichen Schachbretter zu evaluieren und sich
...
Antwort bereitstellen kann, übernimmt die iterative Tiefensuche Funktion mit ihren Evaluierungsfunktionen.
...
{\color{incolor}In [{\color{incolor}25}]:} \PY{n}{board} \PY{o}{=} \PY{n}{chess}\PY{o}{.}\PY{n}{Board}\PY{p}{(}\PY{p}{)}
         ...
         \PY{k}{def} \PY{n+nf}{get\PYZus{}move}\PY{p}{(}\PY{n}{board}\PY{p}{)}\PY{p}{:}
             \PY{c+c1}{\PYZsh{} Selects a random entry for the given position, distributed by the weights of the entries}
             \PY{k}{try}\PY{p}{:}
                 \PY{n}{main\PYZus{}entry} \PY{o}{=} \PY{n}{book}\PY{o}{.}\PY{n}{weighted\_choice}\PY{p}{(}\PY{n}{board}\PY{p}{)}
...
Syzygy ist in die zwei Tabellen \acs{WDL} (\acl{WDL}) und \acs{DTZ} (\acl{DTZ}) unterteilt. Eine weitere Eigenschaft dieser Datenbank ist, dass
